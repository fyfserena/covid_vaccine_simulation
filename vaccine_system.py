...
class vaccine_system(object) : 
    ...
    def recovery(self, re, method="resolve") : 
        ...
        self.requests.extend(new_recovery_citizen)
        ...
        #print("recovery", len(new_recovery_citizen))
        #print(len(self.infected_citizens), len(self.uninfected_citizens), len(self.requests))
    ...
    def start(self) : 
        ...
        return infected_num, uninfected_num, requests_num, anti_body_num
    ...
    def simulate(self, sim_num = 1) : 
        ...
        for i in range(sim_num) : 
            infected_num, uninfected_num, requests_num, anti_body_num = self.start() 
        ...
        #print(infected_nums)
        return infected_nums, uninfected_nums, requests_nums, anti_body_nums
...
def main_sim(parameters : dict) :
    ...
    fig, axes = plt.subplots(2, len(temp_param_values))
    
    for idx, param_value in enumerate(temp_param_values) : 
        temp_params[temp_param_key] = param_value
        env = vaccine_system(**temp_params)
        infected_nums, uninfected_nums, requests_nums, anti_body_nums = env.simulate(sim_num=100)

        datas = [infected_nums, uninfected_nums]
        labels = ["infected_num", "uninfected_num"]
        plotting(datas, labels, days=7, ax=axes[0][idx], title=(temp_param_key, param_value))

        datas = [requests_nums, anti_body_nums]
        labels = ["request_num", "anti_body_num"]
        plotting(datas, labels, days=7, ax=axes[1][idx], title=(temp_param_key, param_value))
